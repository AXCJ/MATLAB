...
% --- Controller design
R = 1e0*eye(ro);
Q = 1e4*eye(po); % Tracker --> the size of Q is po by po
...
% --- Plot fig. --- %
figure(1)
plot(tf, ref_c, '-', tf, y_c, ':', tf, y_o,'-.', 'LineWidth',1.5)
axis([0 Tend -0.8 2.2])
grid on
title('\bf\fontsize{14}\fontname{Cambria} The time responses of \Gamma(t) , y_{c}(t) and y_{o}(t)')
xlabel('\bf\fontsize{10} Time (sec.)')
ylabel('\bf\fontsize{10} Amplitude')
legend('\bf \Gamma','\bf y_{c}','\bf y_{o}')
